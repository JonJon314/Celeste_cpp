...
bool platform_create_window(int width, int height, char* title) {
    ...
    wc.hCursor = LoadCursor(NULL, IDC_ARROW);           // default windows cursor
    wc.lpszClassName = title;                           // not the title but the handle
    wc.lpfnWndProc = windows_window_callback;                    // Callback for window inputs
    ...
    {
        window = CreateWindowExA(0, title, // references lpszClassName form wc
        ...
        if(!pixelFormat) {
            SM_ASSERT(false, "Failed tp choose pixel Format");
            return false;
        }
    ...
    // Actual OpenGL initialization
    {
        ...
        const int pixelAttribs[] = {
            ...
            0 // Terminate with 0, otherwise OpenGL throws error!
        };
